...
function Sharedetail({ value }) {
  ...
  const postDiscuss = async () => {
    const resp = await axios.get(`/getSharePost?unique_code=${pid}`, { headers: headers })
    console.log("iiiii", resp.data.data)
    setpostDiscuss(resp.data.data)
            ...
            {/* {postdiscuss.map((value, index) => ( */}
              <div ref={targetRef}  >
                <div className="imag-text">
                  ...
                  {(!postdiscuss.user_avator == "") ?
                    (
                      <img src={postdiscuss.user_avator} alt='xyz' className="imgprofile" />
                    )
                    :
                    (
                      <Avatar className="avtorsty-nav" name={postdiscuss.name} />
                    )
                  }
                  <div className="under-text-flex">
                    <div className="boths">
                      <h4>{postdiscuss.name}</h4>
                      <h5>{moment(postdiscuss.created_at).format('DD MMM')}</h5>
                    </div>
                  </div>
                </div>
                <h3>{postdiscuss.title}</h3>
                <p>{postdiscuss.description}</p>
                {/* <img
                  src={postdiscuss.post_image}
                  className="tru-img"
                  alt="dicimg5"
                /> */}
